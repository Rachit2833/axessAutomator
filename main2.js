...
async function processPage(page, i) {
  ...
  // let searchedQuestion = mapResponse(radioBox, response)
  // await saveToJSON(searchedQuestion, `action${i}.js`)
  ...
  const responseNested = await getResponse(nestedCheckboxes)
  await saveToJSON(responseNested, "response.js")
  const searchedNestedQuestion = mapResponse(nestedCheckboxes, responseNested)
  await saveToJSON(searchedNestedQuestion, `Nestedaction${i}.js`)
  console.log("waiting for action filling to start");
  await new Promise(resolve => setTimeout(resolve, 2000))
  console.log("startied filling");
  await performActions(page, searchedNestedQuestion)
}
